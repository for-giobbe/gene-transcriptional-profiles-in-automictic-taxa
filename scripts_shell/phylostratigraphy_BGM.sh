...
og=$(grep $i'.*_BGM' Orthogroup_Sequences/*fa | tail -1 | awk -F ":" '{print $1}');

echo -n $og $i" " >> $1.count.tmp

if grep -q PHY $og; then 

if cat $og | tr -d "\n" | grep -qe "BRO.*BAT" -qe "BAT.*BRO"; then echo ubiquitous >> $1.count.tmp; 

else echo shared_with_phyllium >> $1.count.tmp;

fi;

elif cat $og | tr -d "\n" | grep -qe "BRO.*BAT" -qe "BAT.*BRO"; then echo ubiquitous_in_bacillus >> $1.count.tmp;

elif grep -qE '(BRO|BAT)' $og; then echo shared_in_bacillus >> $1.count.tmp; 
